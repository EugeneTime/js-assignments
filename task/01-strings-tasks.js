...
function getRectangleString(width, height) {
  const recWidth = width - 2;
  const recHeight = height - 2;
  const recHeader =  '┌' + '─'.repeat(recWidth) + '┐\n';
  const recMiddle = ('│' + ' '.repeat(recWidth) + '│\n').repeat(recHeight);
  const recFooter =  '└' + '─'.repeat(recWidth) + '┘\n';
  return recHeader + recMiddle + recFooter;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/\w/g, 
    function (str) {
        return symbols.charAt(key.indexOf(str)); 
    } 
  )
}
